...
入侵防御系统不同于杀毒软件和防火墙，三者需要同时应用。对于大中小型企业，防御入侵系统是必不可少的网络安全设备之一。即便是家庭用户也需要这样的设备。  

我开发了基于嵌入式 Linux 的防御入侵系统，有用户测试过下载速度大概有 900Mbps 左右以上。这防御入侵系统功耗极低，平均仅为 5W，运行时静音，体积非常小，只有信用卡大小。硬件设备价格只是数千多元港币左右。与数十万元的企业级防御入侵系统相比，这些设备价格非常实惠。  

安装简单直接，无需设置或调试，即插即用，特别方便适合一般家庭用户或小型公司使用。这些防御入侵设备可达到 1000Mbps 的网络速度，极低的网络延迟让您可以在油管上享受 4K 视频或玩在线游戏，甚至可以使用 BT 下载。  

此外，为了使设备正常运作，整套设备包括有电源适配器和 USB Type-C 线，以及额外两条 CAT 6 网络线。  
...
In addition, in order to make the device operate normally, we provided a power adapter and USB Type-C cable, as well as two additional CAT 6 network cables.
