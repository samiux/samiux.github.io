...
至于 RELRO、NX 和 PIE 可以利用 ROP 技术来绕过限制，但基于一定条件下，例如 elf 位址泄漏。。
...
输入之前建构的字符使程序崩溃，如果程序能够崩溃的话，表明字符足够使程序崩溃，而且程序是存在栈溢出漏洞，之后查看 RBP 的值 (必须是 4 bytes) 然后计算偏移值 ：
...
在一般的情况下，如果一个执行档在任何版本的 libc 库运行，而利用脚本在不经修改的情况下还能够正常运作，这就是所谓可靠的利用 (Reliable Exploit) 了。
